...
export default class GeneratorTool {
    ...
    async #generateTiles(shape, inset, overhang, outsets) {
        ...
        var loop = async(x, y, w, h) => {
            ...
            // while (Collision.polygonPolygon(insetPoints, predictedPoints) && loops < 1) {
                const tile = await this.#generateTile(tempX, y, w, tempY, predictedPoints, inset, overhang, outsets);
                if(tile.length > 0){
                    var actualTile = this.#createTile(tile, true);
                    tmpTiles[xIndex + ", " + yIndex] = actualTile;
                    if(yIndex != 0 && tmpResults.length <= 0){
                        hasTilesAbove = true;
                    }
                    tmpResults.push(actualTile);
                }else {
                    tmpTiles[xIndex + ", " + yIndex] = null;
                }
                loops++;
            // }
            await self.#sleep(10);
        ...
        if(this.debugStartingPoint){
            this.#buffer.fill(255, 0, 0);
            this.#buffer.circle(topleft.x + self.offsetX, topleft.y + self.offsetY, 20);
        }
    ...
    async #generateTile(x, y, w, h, predictedPoints, inset, overhang, outsets){
        ...
        if(points.length > 0)
        {    
            let concaveHull = HullK(points, 5);
            var hull = [];
            for (let i = 0; i < concaveHull.length; i++) {
                const point = concaveHull[i];
                hull.push(new Vector2(point[0], point[1]));
                this.#buffer.vertex(point[0], point[1]);
            }
            return hull;
        } else return [];
    }
